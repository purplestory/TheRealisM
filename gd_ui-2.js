...
function gd_btn_all_menu_open(){
    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            categoryData = {};
            // 이중 배열을 평탄화 (모든 대분류를 categoryData에 넣음)
            $.each(getData, function(i, cateArr) {
                if (Array.isArray(cateArr) && cateArr.length > 0) {
                    $.each(cateArr, function(j, cate) {
                        if (cate && cate.cateCd) {
                            categoryData[cate.cateCd] = cate;
                        }
                    });
                }
            });
            console.log('categoryData:', categoryData); // 값이 잘 나오는지 확인
            if(data =='false') {
...
// === 메가메뉴용 카테고리 데이터 동적 로딩 ===
var categoryData = {};

// 페이지 로딩 시 서버에서 카테고리 전체 구조를 받아옴
...
// === 상단 대분류별 메가메뉴 동적 생성 (칼럼수/분배 개선) ===
$(function() {
  loadCategoryData(function() {
    $('.gnb_menu0').on('mouseenter', 'li', function() {
      var cate = $(this).find('a').data('cate');
      var data = categoryData[cate];
      var html = '';
      // 모든 대분류 비활성화
      $('.gnb_menu0 > li > a').removeClass('active');
      // 현재 대분류만 활성화
      $(this).find('> a').addClass('active');
      if (data && data.children) {
        // 1. 중분류+소분류를 한 배열로 평탄화
        var items = [];
        data.children.forEach(function(medium) {
          items.push({ type: 'medium', name: medium.cateNm });
          if (medium.children) {
            medium.children.forEach(function(small) {
              items.push({ type: 'small', name: small.cateNm, cateCd: small.cateCd });
            });
          }
        });
        // 2. 칼럼 수 결정: 기본 4, 중분류가 5~7개이고 소분류가 거의 없거나 전체 항목이 20개 이하일 때만 5
        var numMedium = data.children.length;
        var numCols = 4;
        if (
          numMedium >= 5 && numMedium <= 7 &&
          items.length <= 20 &&
          data.children.filter(function(m) { return m.children && m.children.length > 0; }).length <= 2
        ) {
          numCols = 5;
        }
        // === 가장 짧은 칼럼에 우선 배분하는 그리디 알고리즘 ===
        var minSmall = 2;
        var columns = Array.from({ length: numCols }, () => []);
        var colLens = Array(numCols).fill(0);
        var colLimit = Math.ceil(items.length / numCols);
        for (var i = 0; i < data.children.length; i++) {
          var medium = data.children[i];
          var group = [{ type: 'medium', name: medium.cateNm }];
          var smalls = [];
          if (medium.children) {
            medium.children.forEach(function(small) {
              smalls.push({ type: 'small', name: small.cateNm, cateCd: small.cateCd });
            });
          }
          var groupLen = 1 + smalls.length;

          // 1. group 전체가 들어갈 수 있는 가장 짧은 칼럼 찾기
          var minIdx = 0;
          for (var c = 1; c < numCols; c++) {
            if (colLens[c] < colLens[minIdx]) minIdx = c;
          }
          // 2. group 전체가 colLimit+2 이하로 들어갈 수 있으면 한 번에 넣음
          if (colLens[minIdx] + groupLen <= colLimit + 2) {
            columns[minIdx] = columns[minIdx].concat(group);
            colLens[minIdx] += 1;
            for (var s = 0; s < smalls.length; s++) {
              columns[minIdx].push(smalls[s]);
              colLens[minIdx]++;
            }
          } else {
            // 3. group이 너무 크면 소분류 일부만 다음 칼럼으로 넘김
            columns[minIdx].push({ type: 'medium', name: medium.cateNm });
            colLens[minIdx]++;
            var s = 0;
            for (; s < smalls.length; s++) {
              if (colLens[minIdx] + 1 > colLimit + 2 && s < smalls.length - minSmall) {
                // 다음 칼럼의 길이가 가장 짧은 칼럼 찾기
                var nextIdx = 0;
                for (var c = 1; c < numCols; c++) {
                  if (colLens[c] < colLens[nextIdx]) nextIdx = c;
                }
                minIdx = nextIdx;
                colLens[minIdx] = columns[minIdx].length;
              }
              columns[minIdx].push(smalls[s]);
              colLens[minIdx]++;
            }
        ...
        // 4. HTML 생성
        html += '<div class="mega-cols">';
        for (var c = 0; c < numCols; c++) {
          html += '<div class="mega-col">';
          var currentGroup = null;
          var currentMediumId = null;
          for (var j = 0; j < columns[c].length; j++) {
            var item = columns[c][j];
            if (item.type === 'medium') {
              if (currentGroup) {
                html += '</div>'; // 이전 그룹 닫기
              }
              currentGroup = true;
              currentMediumId = data.children.find(function(m) { return m.cateNm === item.name; })?.cateCd || '';
              html += '<div class="mega-category-group">';
              html += '<div class="mega-medium" data-medium-id="' + currentMediumId + '">' + item.name + '</div>';
            } else if (item.type === 'small') {
              html += '<div class="mega-small-li" data-medium-id="' + currentMediumId + '"><a href="../goods/goods_list.php?cateCd=' + item.cateCd + '">' + item.name + '</a></div>';
            }
          }
          if (currentGroup) {
            html += '</div>'; // 마지막 그룹 닫기
      ...
      $('.mega-menu').html(html).show();
    });
    $('.gnb_menu_box').on('mouseleave', function() {
      $('.mega-menu').hide();
      // 모든 대분류 비활성화
      $('.gnb_menu0 > li > a').removeClass('active');
    });
  ...
  $(document).on('mousedown', function(e) {
    if ($('.mega-menu').is(':visible')) {
      if (
        ...
        !$(e.target).closest('.gnb_menu0 > li').length
      ) {
        $('.mega-menu').hide();
      }
    }
  });
  // 메뉴 내부 클릭 시 이벤트 버블링 차단
  $(document).on('mousedown', '.mega-menu', function(e) {
    e.stopPropagation();
  });
});
...
// 기존 그룹 hover 이벤트 제거
// 중분류 hover
$(document).on('mouseenter', '.mega-medium', function() {
  $(this).addClass('active');
});
$(document).on('mouseleave', '.mega-medium', function() {
  $(this).removeClass('active');
});
// 소분류 hover
$(document).on('mouseenter', '.mega-small-li', function() {
  var mediumId = $(this).data('medium-id');
  $(this).addClass('active');
  $(this).parent().find('.mega-medium[data-medium-id="' + mediumId + '"]').addClass('active');
});
$(document).on('mouseleave', '.mega-small-li', function() {
  var mediumId = $(this).data('medium-id');
  $(this).removeClass('active');
  $(this).parent().find('.mega-medium[data-medium-id="' + mediumId + '"]').removeClass('active');
});

// 대분류/메가메뉴 모두 벗어났을 때만 .active 해제 (mousemove + 영역 체크)
let menuLeaveTimer = null;

$('.gnb_menu0 > li').on('mouseenter', function() {
  $('.gnb_menu0 > li > a').removeClass('active');
  $(this).find('> a').addClass('active');
  $('.mega-menu').show();
});
...
$(document).on('mousemove', function(e) {
  const $menu = $('.gnb_menu0 > li > a.active').closest('li');
  const $mega = $('.mega-menu');
  const menuRect = $menu.length ? $menu[0].getBoundingClientRect() : null;
  const megaRect = $mega.length ? $mega[0].getBoundingClientRect() : null;
  const x = e.clientX, y = e.clientY;

  let inMenu = false;
  if (menuRect && x >= menuRect.left && x <= menuRect.right && y >= menuRect.top && y <= menuRect.bottom) inMenu = true;
  if (megaRect && x >= megaRect.left && x <= megaRect.right && y >= megaRect.top && y <= megaRect.bottom) inMenu = true;
...
// 메뉴 외부 클릭 시 닫기 (전체메뉴용)
$(document).on('mousedown', function(e) {
    if ($('.gnb_allmenu').is(':visible')) {
        if (
            !$(e.target).closest('.gnb_allmenu').length &&
            !$(e.target).hasClass('btn_all_menu_close')
        ) {
...
// 메뉴 내부 클릭 시 이벤트 버블링 차단 (전체메뉴용)
$(document).on('mousedown', '.gnb_allmenu', function(e) {
    e.stopPropagation();
});